...
fn main() {
  ...
  let (
    source_file_path, // path to source code
    output_file_path, // name of output
    dont_assemble, // if true, stop compilation before assembly code gets assembled
    dont_link, // if true, stop compilation before object file gets linked
    keep_intermediate_files, // if true, don't delete the assembly and object files (intermediate files) after compilation
  ) = cli::handle_args();

  println!("{} {}\n", "Kathleen: compiling".green(), source_file_path.green().italic());
  ...
  for line in fs::read_to_string(source_file_path).unwrap().lines() {
  ...
  let asm_path = &format!("{}.asm", output_file_path);
  let obj_path = &format!("{}.o", output_file_path);
  ...
  if !dont_link && !dont_assemble{
    // run the command `gcc -no-pie output.o -o output -g`
    let mut link = process::Command::new("gcc");
    link.arg("-no-pie").arg(obj_path).arg("-o").arg(output_file_path.clone()).arg("-g");
  ...
  if !issue_during_assembling_or_linking && !dont_assemble && !dont_link {
    ...
    output_file_path.green().italic()
    );
  }

  if !keep_intermediate_files && !dont_assemble && !dont_link && !issue_during_assembling_or_linking {
